...
import { isAuthenticated, hasCompletedOnboarding, createUserProfile, fetchUserProfile, setUser, cleanName, extractNameFromEmail } from '../lib/auth'
      ...
      if (session?.user) {
        ...
        try {
          ...
          if (!profile || !profile.firstName || profile.firstName === 'User') {
            ...
            // Extract name (prioritize robust email parsing for social/placeholder cases)
            let firstName = profile?.firstName && profile.firstName !== 'User' ? profile.firstName : ''
            let lastName = profile?.lastName || ''

            if (!firstName || firstName === 'User') {
              // Try extracting from email first for better splitting (e.g. abdul.mannan)
              const emailParts = extractNameFromEmail(user.email || '')
              firstName = emailParts.firstName
              lastName = emailParts.lastName

              // Fallback to metadata if email extraction resulted in generic 'User' and metadata is available
              if (firstName === 'User') {
                if (metadata.full_name) {
                  const parts = metadata.full_name.split(' ')
                  firstName = cleanName(parts[0]) || 'User'
                  lastName = cleanName(parts.slice(1).join(' ')) || lastName
